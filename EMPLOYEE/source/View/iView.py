...
class EmployeeData_View:
    ...
    def manual_employee_data_entry(self):
        # This function is enable user to \
        # enter employee's data manually
        # Hereby; I have made the employee_data_arr "array"
        # Which will be keep appending as \
        # the user put the data over and over again \
        #
        # Written By: Patel
        #
        emp_data_arr = []
        employee_data = [
                        self.read("Enter Employee ID: "),
                        self.read("Enter gender: "),
                        self.read("Enter age: "),
                        self.read("Enter sales count: "),
    ...
    def display_graph(graph_data):
        # This function is enable to take a graph \
        # object from the graph generator function
    ...
    def employees_by_gender_graph(self, emp_data_arr):
        ...
        # Male and female ratio based on the
        # employee data stored in the system
        #
        # Written By: Patel
        #
        emp_male_count = 0
        ...
        graph_data = {'data': [
                                ...
                                [emp_male_count, emp_female_count],
                                'type': 'pie'}],
            'layout': {'title': 'Number of Employees by Gender'}}
        self.display_graph(graph_data)

    def age_verse_salary_graph(self, emp_data_arr):
        # This function is enable to display \
        # Scatter graph of employee
        # Male vs Female and their salary based on \
        # the data stored in the system
        # The function calculate the actual salary \
        # by deviede the salary by 1000
        # Then pass that information to \
        # the display graph function
        #
        # Written By: Patel
        #
        emp_age_data = []
    ...
    def bmi_pie_graph(self, emp_data_arr):
        ...
        graph_data = {'data': [{'labels': ['Normal', 'Overweight', 'Obesity', 'Underweight'],
                                'values': [
                                    emp_normal_count,
                                    emp_overweight_count,
                                    emp_obesity_count,
                                    emp_underweight_count
                                    ], 'type': 'pie'}],
                                    'layout': {'title': 'Staff by BMI'}}
        self.display_graph(graph_data)
